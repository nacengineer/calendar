defmodule Calendar.Strftime do
  ...
  defp string_for_conv_spec(dt, :a, lang) do
    weekday_abbr(dt, lang)
  end
  ...
  defp string_for_conv_spec(dt, :b, lang) do
    month_abbr(dt, lang)
  end

  defp string_for_conv_spec(dt, :h, lang) do
    string_for_conv_spec(dt, :b, lang)
  end
  ...
  defp string_for_conv_spec(dt, :f, _) do
    micro_seconds(dt)
  end

  defp string_for_conv_spec(dt, :j, _) do
    "#{day_number_in_year(dt)}" |> pad(3)
  end

  defp string_for_conv_spec(dt, :u, _) do
    "#{day_of_the_week(dt)}"
  end

  defp string_for_conv_spec(dt, :w, _) do
    "#{day_of_the_week_zero_sunday(dt)}"
  end
  ...
  defp string_for_conv_spec(dt, :G, _) do
    "#{elem(iso_week_number(dt), 0)}" |> pad(4)
  end

  defp string_for_conv_spec(dt, :g, _) do
    string_for_conv_spec(dt, :G, nil) |> String.slice(-2..-1)
  end

  defp string_for_conv_spec(dt, :y, _) do
    dt = to_date(dt)
    "#{dt.year}" |> String.slice(-2..-1)
  end
  ...
  defp string_for_conv_spec(dt, :T, _) do
    strftime!(dt, "%H:%M:%S")
  end
  ...
  defp string_for_conv_spec(dt, :c, lang),
    do: strftime!(dt, date_time_format_for_lang(lang), lang)
